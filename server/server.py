...
from tts.whisper1 import whisper_speech_recognition, whisper_tts
...
import aiofiles
from tts.wavtomp3 import wav2mp3
...
async def chat_voice(chat_voice:Chat_voice):
    ...
    wav_file_path = os.path.join(USER_VOICE_DIR, f'{userNo}/{file_uuid}.wav')
    
    user_voice_file = base64.b64decode(user_voice)
    
    async with aiofiles.open(wav_file_path, 'wb') as buffer:
        await buffer.write(user_voice_file)
        mp3_file_path = wav2mp3(wav_file_path)
        
    sr_text = whisper_speech_recognition(mp3_file_path)
    ...
    result_file_path = os.path.join(DINO_VOICE_DIR, 'whisper_result.wav')
    
    async with aiofiles.open(result_file_path, "rb") as audio_file:
        voice_result = base64.b64encode(await audio_file.read()).decode('utf-8') 
